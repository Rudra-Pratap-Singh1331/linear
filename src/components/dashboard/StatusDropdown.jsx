...
import { Circle, CircleDashed, XCircle, Check } from "lucide-react";
...
const statusOptions = [
  { id: "backlog", label: "Backlog", icon: CircleDashed, color: "text-zinc-600 dark:text-zinc-500", shortcut: "1" },
  { id: "todo", label: "Todo", icon: Circle, color: "text-zinc-500 dark:text-zinc-300", shortcut: "2" },
  ...
  { id: "canceled", label: "Canceled", icon: XCircle, color: "text-zinc-600 dark:text-zinc-500", shortcut: "5" },
  { id: "duplicate", label: "Duplicate", icon: XCircle, color: "text-zinc-600 dark:text-zinc-500", shortcut: "6" },
];
...
export default function StatusDropdown({ currentStatus, onSelect, onClose }) {
  ...
  return (
    <div
      ref={dropdownRef}
      className="absolute top-[-8px] left-[-8px] w-[260px] rounded-lg z-100 py-1.5 flex flex-col animate-in fade-in zoom-in-95 duration-75 max-h-none overflow-hidden h-auto
      ...
      <div className="px-2 pb-1.5 pt-0.5">
        ...
        >
          <input
            ...
            className="bg-transparent text-[13px] outline-none w-full border-none p-0 focus:ring-0
              text-zinc-900 placeholder-zinc-500
              dark:text-zinc-200 dark:placeholder-zinc-500"
          />

          <div
            className="flex rounded px-1.5 py-0.5 border ml-2 shrink-0
              bg-zinc-300/50 border-zinc-300/60
      ...
      <div className="mt-0.5 overflow-hidden">
        {filteredOptions.map((option) => (
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 transition-colors group text-left
          ...
          >
            <div className="flex items-center gap-3">
              {typeof option.icon === "function" ? (
                ...
                <option.icon size={14} className={cn(option.color)} strokeWidth={2} />
              )}
            ...
            <div className="flex items-center gap-3 shrink-0">
              {currentStatus === option.id && (
                <Check size={14} className="text-zinc-600 dark:text-zinc-400" strokeWidth={2.5} />
              )}
        ...
        {filteredOptions.length === 0 && (
          <div className="px-3 py-4 text-center text-[13px]
            text-zinc-600
            dark:text-zinc-500"
          >
            No results found
