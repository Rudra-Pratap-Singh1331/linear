...
import { Tag, Edit, Calendar, Circle, Reply, Check, SmilePlus, MoreHorizontal, X } from "lucide-react";
...
export default function IssueActivity({ issueId, workspaceId, currentUser, issueTitle, issueDescription }) {
  ...
  useEffect(() => {
    ...
    const fetchActivities = async () => {
      const [commentsResp, eventsResp, issueResp] = await Promise.all([
        ...
        supabase.from("issue_events").select(`*`).eq("issue_id", issueId).order("created_at", { ascending: true }),

        supabase.from("issues").select(`created_at, created_by`).eq("id", issueId).single(),
      ]);
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "issue_comments", filter: `issue_id=eq.${issueId}` }, (payload) => {
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "issue_events", filter: `issue_id=eq.${issueId}` }, (payload) => {
    ...
    switch (eventType) {
      ...
      case "update_status":
        return (
          <span>
            changed status to{" "}
            <span className="font-semibold text-zinc-900 dark:text-zinc-200">{details.to}</span>
          </span>
        );
      ...
      case "update_due_date":
        return (
          <span>
            set due date to{" "}
            <span className="font-semibold text-zinc-900 dark:text-zinc-200">{details.to || "No Date"}</span>
          </span>
        );
      case "add_label":
        return (
          <span>
            added label <span className="font-semibold text-zinc-900 dark:text-zinc-200">{details.label}</span>
          </span>
        );
      case "remove_label":
        return (
          <span>
            removed label <span className="font-semibold text-zinc-900 dark:text-zinc-200">{details.label}</span>
          </span>
        );
      default:
  ...
  const handleSummarize = async (timeframe) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.text) setSummaryData(data.text);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-sm font-semibold text-zinc-800 dark:text-zinc-400">Activity</h3>
        ...
        <div className="relative">
          <button
            ...
            className="
              flex items-center gap-1.5 px-2.5 py-1.5 rounded text-[12px] font-medium transition-colors disabled:opacity-50
              bg-zinc-200/70 hover:bg-zinc-200 text-zinc-900
          ...
          {isSummaryDropdownOpen && (
            <div
              className="
                absolute top-full right-0 mt-2 w-48 rounded-lg shadow-xl py-1 z-50 animate-in fade-in zoom-in-95 duration-100
              ...
              {[
                ...
                <button
                  ...
                  className="
                    ...
                    text-zinc-800 hover:bg-zinc-200/60 hover:text-zinc-950
      ...
      {activities.length === 0 && <div className="text-zinc-600 italic text-xs">No activity yet</div>}
          ...
          {activity.type === "comment" ? (
            <>
              ...
              <div className="flex-1 min-w-0">
                <div
                  className="
                    rounded-md p-3 relative group/bubble transition-colors
                    bg-white border border-zinc-300/50 hover:border-zinc-400/60
                    dark:bg-[#141517] dark:border-white/5 dark:hover:border-white/10
                  "
                >
                  <div
                    className="
                      absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover/bubble:opacity-100 transition-opacity pl-2
                      bg-white
                      dark:bg-[#141517]
                    "
                  >
                    <div className="relative group/reply-btn">
                      ...
                      <div
                        className="
                          absolute bottom-full right-0 mb-2 whitespace-nowrap px-2 py-1 rounded border opacity-0 group-hover/reply-btn:opacity-100 pointer-events-none transition-opacity shadow-xl flex items-center gap-2
                          bg-[#fafafa] text-[11px] text-zinc-800 border-zinc-300/60
                          dark:bg-zinc-800 dark:text-zinc-300 dark:border-white/10
                        "
                      >
                        Reply to comment{" "}
                        <span className="bg-zinc-200 text-zinc-900 dark:bg-zinc-700 dark:text-zinc-100 px-1 rounded text-[10px] font-bold">
                          R
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 text-[13px] mb-1">
                    <span className="font-semibold text-zinc-900 dark:text-zinc-200">{activity.user}</span>
                    <span className="text-zinc-500 dark:text-zinc-500">{timeAgo(activity.created_at)}</span>
                  </div>

                  <div className="text-[14px] text-zinc-800 dark:text-zinc-300 leading-relaxed whitespace-pre-wrap">
                    {activity.text}
                  </div>
                </div>
                ...
                {replyingToId === activity.id && (
                  <div className="mt-3 ml-0 space-y-3 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div
                      className="
                        rounded-lg p-3 shadow-lg group transition-colors
                        bg-[#fafafa] border border-zinc-300/60
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <textarea
                            ...
                            className="
                              w-full bg-transparent border-none p-0 text-[14px] resize-none min-h-[40px] leading-relaxed overflow-hidden outline-none
                          ...
                          <div className="flex items-center justify-between mt-3 pt-2 border-t border-zinc-300/40 dark:border-white/5">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className="
                                  flex items-center gap-1.5 px-2.5 py-1.5 rounded text-[12px] font-medium transition-all disabled:opacity-30
            ...
            <>
              <div className="w-6 h-6 flex items-center justify-center text-zinc-600 dark:text-zinc-500 shrink-0 relative">
                <div className="w-px h-full bg-zinc-300/40 dark:bg-zinc-800 absolute top-[-12px] bottom-[-12px] -z-10 group-first:top-3 group-last:bottom-3 hidden" />
                {getEventIcon(activity.eventType)}
              </div>

              <div className="flex-1 min-w-0 flex items-center text-[13px] text-zinc-700 dark:text-zinc-400">
                <span className="font-semibold text-zinc-900 dark:text-zinc-200 mr-1">{activity.user}</span>
      ...
      {summaryData && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-100 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div
            className="
              w-full max-w-lg shadow-2xl overflow-hidden rounded-xl animate-in zoom-in-95 duration-200
              bg-[#fafafa] border border-zinc-300/60
              dark:bg-[#141517] dark:border-white/10
            "
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-300/40 dark:border-white/5">
              ...
              <button onClick={() => setSummaryData(null)} className="text-zinc-600 hover:text-zinc-950 dark:text-zinc-500 dark:hover:text-zinc-300 transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-6">
              <div
                className="
                  text-[14px] leading-relaxed whitespace-pre-wrap rounded-lg max-h-[400px] overflow-y-auto custom-scrollbar p-4
                  bg-zinc-200/60 border border-zinc-300/50 text-zinc-900
                  dark:bg-white/5 dark:border-white/5 dark:text-zinc-300
                "
              >
                {summaryData}
              </div>
            ...
            <div className="px-6 py-4 border-t border-zinc-300/40 dark:border-white/5 flex justify-end">
              <button
